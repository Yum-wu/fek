"""图编译器 —— 将（策略, 任务）编译为可运行的执行图（ExecutionGraph）。
...
from ..core.graph import ExecutionGraph, GraphNode
...
class GraphBuilder:
    ...
    def build(self, strategy: Strategy, task: Task) -> ExecutionGraph:
    ...
    def _single(self, task: Task) -> ExecutionGraph:
        # 单模型：仅一个求解节点
        g = ExecutionGraph()
    ...
    def _multi_agent(self, task: Task) -> ExecutionGraph:
    ...
    def _moa(self, task: Task) -> ExecutionGraph:
