...
REM ============================================================
REM  FEK · Fusion Execution Kernel —— 本地测试一键启动器
REM  用法：双击本文件，按菜单选择即可。
REM  依赖：Python 3.10+（Demo / 测试零依赖；Web 界面会自动安装 streamlit）
REM ============================================================

REM 定位 Python 解释器（依次尝试 python / py / 托管路径）
set "PY="
...
if not defined PY (
  set "PY=C:\Users\Yum\.workbuddy\binaries\python\versions\3.13.12\python.exe"
)
if not defined PY (
  echo [错误] 未找到 Python。请先安装 Python 3.10+ 并加入 PATH，或编辑本脚本中的 PY 路径。
  pause
  exit /b 1
)
